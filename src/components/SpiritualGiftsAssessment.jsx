...
import { ChevronRight, ChevronLeft, Award, Heart, BookOpen, MessageCircle, Gift, Users, Handshake, Compass } from 'lucide-react';
...
export default function SpiritualGiftsAssessment() {
  ...
  const saveResults = async (sortedGifts) => {
    ...
    try {
      ...
      setSaveError(error.message || 'Failed to submit results.');
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            {saveStatus === 'saving' && (
              <p>Saving your results to the team spreadsheet...</p>
            )}
            {saveStatus === 'saved' && (
              <p>Your results have been saved and shared with the assessment administrator.</p>
            )}
            {saveStatus === 'failed' && (
              <p style={{ color: '#b71c1c' }}>
                We could not save your results right now. {saveError}
              </p>
            )}
          </div>
        </div>
